...
describe("isValid", () => {
    ...
    it("Retorna verdadeiro quando .pendente e .total são zero", () => {
      ...
      expect(isValid(situacao, "PRE_NATAL_6_CONSULTAS" as Indicadores)).toBe(true);
    });

    it("Retorna falso quando .pendente e .total são null", () => {
      situacao = { ...situacao, PRE_NATAL_ODONTO: { total: null, pendente: null } };
      expect(isValid(situacao, "PRE_NATAL_ODONTO" as Indicadores)).toBe(false);
    });
    ...
    it("Retorna falso quando .pendente e .total são undefined", () => {
      ...
      expect(isValid(situacao, "PRE_NATAL_SIFILIS_HIV" as Indicadores)).toBe(false)
});
});