...
const coapsUrl = `http://localhost:3000/api/cofin25/indicadores/cuidado_da_pessoa_com_hipertensao/coaps/dados`;
const body = {} satisfies schema.CoapsPageRequestBody;
const stringfiedBody = JSON.stringify(body);

describe(`api/cofin25/indicadores/cuidado_da_pessoa_com_hipertensao/coaps/dados Route Handler`, () => {
    beforeEach(() => {
        jest.clearAllMocks();
        jest.resetModules();
    });
    ...
    describe(`POST api/cofin25/indicadores/cuidado_da_pessoa_com_hipertensao/coaps/dados`, () => {
        it("Deve retornar 404 se a feature flag hypertensionNewProgram não estiver habilitada", async () => {
            ...
            const { POST } = await import(
                "@/app/api/cofin25/indicadores/cuidado_da_pessoa_com_hipertensao/coaps/dados/route"
            );
