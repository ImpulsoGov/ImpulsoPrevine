...
const coeqUrl = `http://localhost:3000/api/cofin25/indicadores/cuidado_da_pessoa_com_diabetes/coeq/dados`;
const body = {
    search: "Paciente C",
} satisfies schema.CoeqPageRequestBody;
const stringfiedBody = JSON.stringify(body);

describe(`api/cofin25/indicadores/cuidado_da_pessoa_com_diabetes/coeq/dados Route Handler`, () => {
    beforeEach(() => {
        jest.clearAllMocks();
        jest.resetModules();
    });
    ...
    describe(`POST api/cofin25/indicadores/cuidado_da_pessoa_com_diabetes/coeq/dados`, () => {
        it("Deve retornar 404 se a feature flag diabetesNewProgram não estiver habilitada", async () => {
            ...
            const { POST } = await import(
                "@/app/api/cofin25/indicadores/cuidado_da_pessoa_com_diabetes/coeq/dados/route"
            );
        ...
        it("Deve retornar 403 se o usuário não possuir o perfil permitido na rota", async () => {
            ...
            authHelpers.mockDecodeToken().mockResolvedValue(
                authHelpers.decodedToken({
                    payload: { perfis: [PROFILE_ID.userManagement] },
                })
            );
            dbHelpers.mockPrismaClient();
