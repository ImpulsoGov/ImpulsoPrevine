...
const patientCpfOrBirthdayAdapter = (
    patientCpfOrBirthdayString: string
): string | Date | null => {
    if (
        patientCpfOrBirthdayString.length === 11 &&
        !patientCpfOrBirthdayString.includes("-")
    )
        return patientCpfOrBirthdayString;
    if (
        patientCpfOrBirthdayString.length === 10 &&
        patientCpfOrBirthdayString.includes("-")
    )
        return new Date(patientCpfOrBirthdayString);
    return null;
};
...
export const diabetesAcfPrintForTeamDataAdapter = (
    ...
    return data.map((item) => ({
        ...
        hemoglobinTestDueDate:
            item.prazo_proxima_solicitacao_hemoglobina_glicada,
        nextAppointmentDueDate: item.prazo_proxima_consulta,
        patientStatus: item.status_usuario as PatientStatus,
        conditionIndentifiedBy:
            item.identificacao_condicao_diabetes as ConditionIdentifiedBy,
        patientCpfOrBirthday: patientCpfOrBirthdayAdapter(
            item.cidadao_cpf_dt_nascimento
        ),
