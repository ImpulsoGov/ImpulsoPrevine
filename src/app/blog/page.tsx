...
const HomeBlogPage = async() => {
  ...
  const artigos = artigosData ? artigosData.blogArtigos.map((artigo : any)=>{
    return (
          {
            id: artigo.id,
            tag : artigo.tag,
  ...
  const artigoDestaque = artigos.sort((a : any,b : any) => new Date(b.data).getTime() - new Date(a.data).getTime())[0];
