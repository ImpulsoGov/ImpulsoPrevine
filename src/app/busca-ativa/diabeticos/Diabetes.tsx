...
export const Diabetes : React.FC<DiabetesProps> = ({
    ...
    useEffect(() => {router.push(`${path}?aba=${''}&sub_aba=${''}&visao=${visao}`)}, [visao]);
    const Voltar = () => window.history.go(voltarGatilho * (-2))
    useEffect(() => {setVoltarGatilho(voltarGatilho + 1)}, [path])

    if (!session) return <Spinner/>
    
    if (session.user.perfis.includes(9)) return <DiabetesEquipe
    ...
    Voltar={Voltar}
    />
    if (session.user.perfis.includes(5) || session.user.perfis.includes(8)) return <DiabetesAPS
