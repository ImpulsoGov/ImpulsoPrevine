	...
	return (
		<div>
			<div className={styles.textContainer}>
				<span>
					Confira aqui sua evolução nos cadastros, considerando a produção de:
					&nbsp;{" "}
				</span>
				<div className={styles.selectorBox}>
					...
					{showCheckboxes && (
						<div className={styles.checkboxes}>
							{[...new Set(GrafEvolucao.map((item) => item.equipe_status))].map(
								(indicador, index) => (
									<label key={index}>
										<input
											type="checkbox"
...
const GraficoEvolucaoEquipe = ({ GrafEvolucao }) => {
	...
	useEffect(() => {
		...
		const periodos = [
			...new Set(selectedData.map((item) => item.data_inicio)),
		].sort();

		const newSeries = ["cadastro_total", "cadastros_com_pontuacao"].map(
			(indicador) => {
	...
	const option = {
		...
		legend: {
			data: [
				"Cadastros Total",
				"Cadastros com Ponderação",
				"Parâmetro municipal mais recente",
			],
			top: 25,
			left: "center",
			orient: "horizontal",
