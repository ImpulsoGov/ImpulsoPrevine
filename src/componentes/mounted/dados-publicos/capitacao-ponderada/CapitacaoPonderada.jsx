...
export const CapitacaoPonderada = ({ cidade }) => {
	...
	useEffect(() => {
		CadastrosEquipes(cidade).then((result) => setCaptacaoEvolucao(result));
	}, [cidade]);

	const [CapValidacaoProducao, setCapValidacaoProducao] = useState([]);
	useEffect(() => {
		ValidacaoProducao(cidade).then((result) => setCapValidacaoProducao(result));
	}, [cidade]);

	const [CapValidacaoProducaoAplicada, setCapValidacaoProducaoAplicada] =
		useState([]);
	useEffect(() => {
		if (cidade != null)
	...
	const quantidadeCadastrosPreliminaresInvalidos = CapValidacaoProducao.reduce(
		(total, item) => {
	...
	return (
		<div style={{ margin: "0px 80px" }}>
			...
			/>
			<GraficoSuasEquipes GrafCapitacao={indicadoresData} />
			...
			/>
			<GraficoEvolucaoEquipe GrafEvolucao={CaptacaoEvolucao} />
			...
			<TituloSmallTexto
				...
				titulo="<b>Desempenho das equipes</b>"
				tooltip=""
			/>

			<TabelaDesempenhoEquipes TabDesempenhos={CaptacaoEvolucao} />
			...
			<GraficoFichaProducao GrafFicha={CapValidacaoProducaoAplicada} />
			...
			<TabelaFichasCadastro TabFichas={CapValidacaoProducaoAplicada} />
			...
			<TabelaCadastroPreliminar TabCadPreliminar={CapValidacaoProducao} />
