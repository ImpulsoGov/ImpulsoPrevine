			...
			<div className={styles.selectorBox}>
				...
				{showCheckboxes && (
					<div className={styles.checkboxes}>
						{[...new Set(GrafHistorico.map((item) => item.indicador_nome))].map(
							(indicador, index) => (
								<label key={index}>
									<input
										type="checkbox"
...
const GraficoHistoricoDesempenho = ({ GrafHistorico }) => {
	...
	useEffect(() => {
		...
		const periodos = [
			...new Set(selectedData.map((item) => item.periodo_codigo)),
		].sort();
	...
	const option = {
		...
		xAxis: {
			...
			boundaryGap: false,
			data: [
				...new Set(GrafHistorico.map((item) => item.periodo_codigo)),
			].sort(),
			axisLabel: {
				rotate: 45,
			},
