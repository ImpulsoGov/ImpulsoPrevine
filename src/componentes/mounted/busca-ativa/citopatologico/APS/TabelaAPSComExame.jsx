...
export const TabelaAPSComExame = ({
	tabelaDataAPS,
	tabelaData,
	setTabelaData,
	mixpanel,
	...
	const tabelaDataAPSComExame = [
		...new Set(tabelaDataAPS.filter((item) => item.id_status_usuario === 12)),
	].map((item) => ({
		...item,
		...
		<>
			<PainelBuscaAtiva
				...
				dadosFiltros={[
					{
						data: [
							...new Set(tabelaDataAPSComExame.map((item) => item.acs_nome)),
						],
						filtro: "acs_nome",
					...
					},
					{
						data: [
							...new Set(
								tabelaDataAPSComExame.map((item) => item.equipe_nome_e_ine),
							),
						],
