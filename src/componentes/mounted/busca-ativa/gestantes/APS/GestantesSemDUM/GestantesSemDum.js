...
const TabelaGestantesSemDUM = ({
    ...
    const tabelaDataAPSGestantesSemDUM = tabelaDataAPS
        ?.filter((item) => item.id_status_usuario == 11)
        .map((item) => ({
            ...item,
    ...
    return tabelaDataAPS ? (
        <PainelBuscaAtiva
            ...
            dadosFiltros={[
                {
                    data: [
                        ...new Set(
                            tabelaDataAPSGestantesSemDUM.map(
            ...
            atualizacao={new Date(
                tabelaDataAPSGestantesSemDUM.reduce((maisRecente, objeto) => {
