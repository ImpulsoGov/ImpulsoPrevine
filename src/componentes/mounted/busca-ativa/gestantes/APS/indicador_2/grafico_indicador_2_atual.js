...
const CardsGraficoIndicadorDoisQuadriAtual = ({ tabelaDataAPS }) => {
	...
	return tabelaDataAPS ? (
		<>
			...
			>
				{quadriAtualFormatado && `${quadriAtualFormatado} -`} Gestantes com DUM
				preenchida por exames de Sífilis e HIV identificados por equipe de saúde
			</h2>
			<ScoreCardGrid
				valores={[
					{
						descricao:
							"Gestantes com os dois exames realizados e identificados",
					...
					},
					{
						...
						valor: dataQuadriAtual.reduce((acumulador, item) => {
							return (item.id_status_usuario == 8 ||
								item.id_status_usuario == 9) &&
								(item.id_exame_hiv_sifilis == 1 ||
									item.id_exame_hiv_sifilis == 2)
								? acumulador + 1
								: acumulador;
						}, 0),
					},
		...
		<>
			<GraficoBuscaAtiva
				dataBarra={{
					...
					color: ["#2EB280", "#EF565D", "#E98633", "#7579EA"],
					grid: {
						containLabel: true,
						top: "20%",
					},
					...
					series: [
						{
							data: Object.entries(
								dataQuadriAtual.reduce((acumulador, item) => {
									if (item.id_exame_hiv_sifilis == 4)
						...
						},
						{
							data: Object.entries(
								dataQuadriAtual.reduce((acumulador, item) => {
									if (item.id_exame_hiv_sifilis == 3)
										acumulador[item.equipe_nome] =
											(acumulador[item.equipe_nome] || 0) + 1;
									return acumulador;
								}, {}),
						...
						},
						{
							data: Object.entries(
								dataQuadriAtual.reduce((acumulador, item) => {
									if (item.id_exame_hiv_sifilis == 1)
										acumulador[item.equipe_nome] =
											(acumulador[item.equipe_nome] || 0) + 1;
									return acumulador;
								}, {}),
						...
						},
						{
							data: Object.entries(
								dataQuadriAtual.reduce((acumulador, item) => {
									if (item.id_exame_hiv_sifilis == 2)
										acumulador[item.equipe_nome] =
											(acumulador[item.equipe_nome] || 0) + 1;
									return acumulador;
								}, {}),
				...
				dataRosca={{
					...
					color: ["#EF565D", "#2EB280", "#E98633", "#7579EA"],
					series: [
						{
							...
							data: [
								{
									...
									value: (
										(dataQuadriAtual.reduce((acumulador, item) => {
											return item.id_exame_hiv_sifilis == 3
												? acumulador + 1
												: acumulador;
										}, 0) *
											100) /
								...
								},
								{
									...
									value: (
										(dataQuadriAtual.reduce((acumulador, item) => {
											return item.id_exame_hiv_sifilis == 1
												? acumulador + 1
												: acumulador;
										}, 0) *
											100) /
								...
								},
								{
									...
									value: (
										(dataQuadriAtual.reduce((acumulador, item) => {
											return item.id_exame_hiv_sifilis == 2
												? acumulador + 1
												: acumulador;
										}, 0) *
											100) /
