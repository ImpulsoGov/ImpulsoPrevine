...
const paddingCalc = (verticalDivider: number[], index: number) => 
    [...verticalDivider.map(item=>item+1),0].includes(index) ? "5px 5px 5px 12px" : "5px"
...
<tr 
  ...
  {columns.map((column,index)=>{
    return <th style={{
      ...
      borderTopRightRadius: index!==(columns.length-1) ? "0" : "8px",
      borderBottomLeftRadius: index!==0 ? "0" : "8px",
      borderBottomRightRadius: index!==(columns.length-1) ? "0" : "8px",
      ...
      {column.headerComplement && (
        <div style={{
          marginBottom: "6px",
          visibility: column.visibleHeaderComplement ? "visible" : "hidden",
        }}>
          {column.headerComplement}
        </div>
      )}
