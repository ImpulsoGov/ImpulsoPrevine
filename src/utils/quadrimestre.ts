...
export const obterDadosQuadrimestre = (data: Date | string | number) => {
	const objetoDeData =
		typeof data == "string" || typeof data == "number" ? new Date(data) : data;
...
export const obterDadosProximosQuadrimestres = (
	...
	for (let iteracao = quantidade; iteracao > 0; iteracao--) {
		const dadosUltimoQuadrimestre: QuadrimestreData =
			proximosQuadrimestres.length === 0
				? obterDadosQuadrimestre(dataBase)
				: (proximosQuadrimestres.at(-1) as QuadrimestreData);

		const proximoQuadrimestre =
			Number(dadosUltimoQuadrimestre.quadrimestre) + 1;
...
export const formatarQuadrimestres = (quadrimestres: any, separador = "") => {
	const quadrisPorAno = quadrimestres.reduce(
		(acc: any, { quadrimestre, ano }: any) => {
			const quadri = `Q${quadrimestre}`;

			acc[ano] ? (acc[ano] = `${acc[ano]} + ${quadri}`) : (acc[ano] = quadri);
