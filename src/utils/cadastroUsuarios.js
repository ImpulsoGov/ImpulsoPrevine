...
export const Validacao = async (data) => {
    ...
    const ValidarMunicipioExistente = (municipio) =>
        MUNICIPIOS.some(({ nome, uf }) => `${nome} - ${uf}` === municipio);
    ...
    const ValidarTelefone = (numero) =>
        /^\d{11}$/.test(numero) && ValidarExistencia(numero);
    const ValidarWP = (wp) => (wp == "1" || wp == "0") && ValidarExistencia(wp);
    const ValidarMunicipio = (municipio) =>
        ValidarExistencia(municipio) && ValidarMunicipioExistente(municipio);
