...
jest.mock("@features/common/shared/migratedMunicipalities", () => ({
    isMigratedMunicipality: jest.fn(),
}));

const getTokenMock = getToken as jest.MockedFunction<typeof getToken>;
const isMigratedMunicipalityMock =
    isMigratedMunicipality as jest.MockedFunction<
        typeof isMigratedMunicipality
    >;
...
describe("middlewarePages: redirecionamento para o Portal", () => {
    ...
    it("manda o município migrado para o Portal, marcando a origem", async () => {
        isMigratedMunicipalityMock.mockResolvedValue(true);
    ...
    it("deixa o município não migrado seguir no IP", async () => {
        isMigratedMunicipalityMock.mockResolvedValue(false);
    ...
    it("não redireciona prefetch: só navegação de verdade", async () => {
        ...
        expect(resposta.headers.get("location")).toBeNull();
        expect(isMigratedMunicipalityMock).not.toHaveBeenCalled();
    });
