...
const TabelaCadastroPreliminarcomSeletor = ({
  ...
  const handleCheckboxChange = (value) => {
    ...
    if (updatedSelectedIndicadores.length === 0) {
      ...
      setSelectedNovoIndicador([...new Set(TabCadPreliminar
        .filter((item) => item.validacao_nome === "Preliminar>Reprovado(PROF)")
        ...
        <div className={`${styles.selectorBox} ${styles.equipeSelector}`}>
          ...
          {showCheckboxes && (
            <div className={styles.checkboxes}>
              ...
              {selectedIndicadores
                .filter((indicador, index, self) => self.indexOf(indicador) === index)
                .map((indicador, index) => (
...
const TabelaCadastroPreliminar = ({ TabCadPreliminar }) => {
      ...
      .map((item) => {
        ...
        if (!linhasUnicas.has(chave)) {
          ...
          return {
            ...
            validacao_quantidade: calculateQuantidade(item.equipe_nome),
          };
        }
        return null;
      })
  ...
  useEffect(() => setSelectedIndicadores([...new Set(TabCadPreliminar
    .filter((item) => item.validacao_nome === "Preliminar>Reprovado(PROF)")
    .map((item) => item.equipe_nome))]), [TabCadPreliminar]);
  useEffect(() => setSelectedNovoIndicador([...new Set(TabCadPreliminar
    .filter((item) => item.validacao_nome === "Preliminar>Reprovado(PROF)")
    .map((item) => item.periodo_data_inicio))]), [TabCadPreliminar]);
