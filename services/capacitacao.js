...
const consultarAvaliacaoConclusaoPorUsuarioClient = async(usuario_id,token)=>{
    try {
        let data = new FormData();
        data.append('usuario_id', usuario_id);
        let config = {
            method: 'post',
            maxBodyLength: Infinity,
            url:  API_URL + 'impulsoprevine/capacitacao/consulta-avaliacao-conclusao-por-usuario',
            headers: { 
                'Authorization': 'Bearer ' + token, 
              },
            data : data
...
export {consultarAvaliacaoConclusao,concluirConteudo,avaliarConteudo,consultarAvaliacaoConclusaoPorUsuario}